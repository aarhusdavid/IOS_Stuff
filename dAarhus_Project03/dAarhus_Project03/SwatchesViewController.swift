...
extension SwatchesViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier:"SlideUpViewCell", for: indexPath)
            as? SlideUpViewCell
            else {
        ...
        indexPath: IndexPath)
    {
        ...
        {
            ColorManager.colorCollection.remove(at:
                currentlySelectedSwatch.row)
            self.MainViewController.deleteItems(at:
                [currentlySelectedSwatch])
            self.MainViewController.reloadItems(at:
                self.MainViewController.indexPathsForVisibleItems)
            StorageHandler.set()
            slideUpViewTapped()
        }else if indexPath.row == 1 {
            let shareText = "Check out my color: #" +
                ColorManager.colorCollection[currentlySelectedSwatch.row].GetHex()
...
extension SwatchesViewController {
    ...
    let cellColor = UIColor(red:
        CGFloat(cellColorArray.red)/255, green:
        CGFloat(cellColorArray.green)/255, blue:
        CGFloat(cellColorArray.blue)/255, alpha:
        CGFloat(cellColorArray.alpha)/255)
    ...
    {
        // this ensures it only fires once when it begins
...
extension SwatchesViewController {
  override func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
    ...
    colorTab.redSlider.value = Float(cellColorArray.red)
    colorTab.greenSlider.value = Float(cellColorArray.green)
    colorTab.blueSlider.value = Float(cellColorArray.blue)
    colorTab.alphaSlider.value = Float(cellColorArray.alpha)
